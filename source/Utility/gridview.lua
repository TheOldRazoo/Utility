...
gridview.__index = function(table, key)
	...
	elseif key == "easingPeriod" then
		...
		return rawget(gridview, key)
	end
end


gridview.__newindex = function(table, key, value)
...
function gridview.new(cellWidth, cellHeight)
	...
	local o = {}
	setmetatable(o, gridview)

	o.userData = nil
	o.userDrawCellCallback = nil
...
function gridview:getUserData()
    return self.userData
end

function gridview:setUserData(data)
    self.userData = data
end

function gridview:getUserDrawCellCallback()
    return self.userDrawCellCallback
end

function gridview:setUserDrawCellCallback(callback)
...
function gridview:drawHorizontalDivider(x, y, width, height)
	gfx.setColor(gfx.kColorBlack)
	local lineHeight = 2
...
function gridview:setHorizontalDividerHeight(newHeight)
	assert(newHeight ~= nil, "nil value passed to gridview.setHorizontalDividerHeight()")
	assert(newHeight >= 0, "negative value passed to gridview.setHorizontalDividerHeight()")
	self.horizontalDividerHeight = newHeight
	self.needsDisplay = true
end

function gridview:getHorizontalDividerHeight()
	return self.horizontalDividerHeight
end


function gridview:removeHorizontalDividers()
	self.horizontalDividers = {}
	self.needsDisplay = true
end


function gridview:addHorizontalDividerAbove(section, row)
...
function gridview:getCellBounds(section, row, column, gridWidth)

	assert(self.cellWidth ~= 0 or gridWidth ~= nil, "gridview:getCellBounds() requires `gridWidth` argument when cell width is zero")
			...
			for row = 1, self.numRows[section] do
				...
				for col = 1, self.numColumns do
					...
					if drawY + self.cellHeight > 0 and drawY < height and drawX + cellWidth > 0 and drawX < width then
						...
						if self.userDrawCellCallback then
							self.userDrawCellCallback(self, section, row, col, selected, floor(x + 		drawX), floor(y + drawY), cellWidth, self.cellHeight)
						else
...
function gridview:setNumberOfColumns(num)
	if num > 0 then
		self.numColumns = num
	else
		print("ERROR: gridview must be at least one column wide.")
	end
...
function gridview:getNumberOfColumns()
	return self.numColumns
end


function gridview:setNumberOfSections(num)
	...
	if num > 0 then
		...
		print("ERROR: gridview must contain at least one section.")
	end

	self.needsDisplay = true
end

function gridview:getNumberOfSections()
	return self.numSections
end


function gridview:setNumberOfRowsInSection(section, num)
	if num > 0 then
		self.numRows[section] = num
		if self.numSections < section then self:setNumberOfSections(section) end
	else
		print("ERROR: gridview sections must contain at least one row.")
	end

	self.needsDisplay = true
end

function gridview:setNumberOfRows(...)
...
function gridview:getNumberOfRowsInSection(section)
...
function gridview:setSectionHeaderHeight(height)
...
function gridview:getSectionHeaderHeight()
	return self.sectionHeaderHeight
end

function gridview:setContentInset(left, right, top, bottom)
	assert(left ~= nil and right ~= nil and top ~= nil and bottom ~=nil, "nil value passed to gridview:setContentInset()")
...
assert(left ~= nil and right ~= nil and top ~= nil and bottom ~=nil, "nil value passed to gridview:setCellPadding()")
...
function gridview:setSectionHeaderPadding(left, right, top, bottom)
assert(left ~= nil and right ~= nil and top ~= nil and bottom ~=nil, "nil value passed to gridview:setSectionHeaderPadding()")
...
function gridview:setScrollPosition(x, y, animated)
...
function gridview:getScrollPosition()
...
function gridview:scrollToCell(section, row, column, animated)

	if animated == nil then animated = true end

	if deferScroll(self, gridview.scrollToCell, section, row, column, animated) then
		return
	end
...
function gridview:scrollCellToCenter(section, row, column, animated)

	if animated == nil then animated = true end

	if deferScroll(self, gridview.scrollCellToCenter, section, row, column, animated) then
		return
	end
...
function gridview:scrollToRow(row, animated)
	if animated == nil then animated = true end
	if self.scrollCellsToCenter then
...
function gridview:scrollToTop(animated)
	if animated == nil then animated = true end
...
-- tableview convenience function - assumes only one section and one column
function gridview:setSelectedRow(row)
	self:setSelection(1, row, 1)
end

function gridview:getSelectedRow()
...
function gridview:selectNextRow(wrapSelection, scrollToSelection, animate)
...
function gridview:selectPreviousRow(wrapSelection, scrollToSelection, animate)
...
function gridview:selectNextColumn(wrapSelection, scrollToSelection, animate)
...
function gridview:selectPreviousColumn(wrapSelection, scrollToSelection, animate)
...
function gridview:setScrollDuration(ms)
